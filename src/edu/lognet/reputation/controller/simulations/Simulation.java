...
public abstract class Simulation {
	...
	/* Constructors */
	/* --------------------------------------------------------- */
	public Simulation(int interactionNumber2, int serviceNumber2, int totalUserNumber, int goodUser2, int goodTurnBadUser, int fluctuateUser, int frequencyOfFluctuation, int badUser2, int badTurnGoodUser, int honestRater, int dishonestRater, int randomRater, int collusiveGroupNum, int resourceAvailable, int dataLost, int chosenStrategy2) {
		...
		this.normalUser = 100 - (goodUser2+goodTurnBadUser+fluctuateUser+badUser2+badTurnGoodUser);
	...
	public List<User> getUserSet(List<Service> services) {
		...
		for (int i = 0; i < userNumber; i++) {
			...
			*/
			...
			if ((((100 * good) / userNumber) < goodUser) && (rand<(goodUser+normalUser/5))) {
				QoS = (randomGenerator.nextInt(30) + 70.0) / 100;
				good++;
				pType = User.providerType.GOOD;
			} 
			//create goodTurnBadUser
			else if ((((100 * goodTB) / userNumber) < goodTurnBadUser)&&(rand>=(goodUser+normalUser/5))&&(rand<(goodUser+goodTurnBadUser+2*normalUser/5))) {
				QoS = (randomGenerator.nextInt(30) + 70.0) / 100;
				goodTB++;
				pType = User.providerType.GOODTURNSBAD;
			} 
			//create fluctuateUser whose QoS is dynamic and set later, initially can be random
			else if ((((100 * fluctuate) / userNumber) < fluctuateUser)&&(rand>=(goodUser+goodTurnBadUser+2*normalUser/5))&&(rand<(goodUser+goodTurnBadUser+fluctuateUser+3*normalUser/5))) {
			...
			//Bad users
			else if ((((100 * bad) / userNumber) < badUser)&&(rand>=(goodUser+goodTurnBadUser+fluctuateUser+3*normalUser/5))&&(rand<(goodUser+goodTurnBadUser+fluctuateUser+badUser+4*normalUser/5))) {
				QoS = (randomGenerator.nextInt(30) + 1.0) / 100;
				bad++;
				pType = User.providerType.BAD;
			} 
			//badTurnGoodUser
			else if ((((100 * badTG) / userNumber) < badTurnGoodUser)&&(rand>=(goodUser+goodTurnBadUser+fluctuateUser+badUser+4*normalUser/5))&&(rand<(goodUser+goodTurnBadUser+fluctuateUser+badUser+badTurnGoodUser+normalUser))) {
				QoS = (randomGenerator.nextInt(30) + 1.0) / 100;
				badTG++;
				pType = User.providerType.BADTURNSGOOD;
			}
			//normal, size can be bigger than it's supposed to be and then some other group is fewer than it's supposed to be.
			else {
			...
			//The order of raterType assigning doesn't matter to providers since
			...
			//collusiveRater
				...
				else if (((100*counter[1]/userNumber) < sizeOfEachGroup)||(collusiveGroupNum==2)){
					counter[1]++;
			...
			User user = new User("u" + i, "user" + i, age, service, pType, rType, cGroup, QoS, resourceAvailable, frequencyOfFluctuation);
			users.add(user);
